...
// 			pStreamCtx->RefCount -- ;
		...
		//获取file flag首位置偏移
		ByteOffset.QuadPart = FileSize.QuadPart - FILE_FLAG_LENGTH ; //JK 如果文件大小是200，这里减少个8，就是192，作用是读位于文件尾部的文件头
		...
		if (FILE_GUID_LENGTH != RtlCompareMemory(g_psFileFlag, psFileFlag, FILE_GUID_LENGTH))
		{
			...
			//这里的意思是如果文件在关闭时写入了文件头，就不会走到这里并在下面设置为加密~ 说明这是一个刚创建的文件，具体是不是加密文件还要等在关闭时来设置~
			SC_LOCK(pStreamCtx, &OldIrql) ;
...
{   
	...
	try{
		...
		{// not monitored, pass
				...
				__leave ;//目前总是从这里跳出了，并未走到下面的bHasWriteData判断，但是手动成功那次确实走到了下面的bHasWriteData判断。
		...
		DbgPrint("In PreClose A pStreamCtx->RefCount = %d \n",pStreamCtx->RefCount);
		...
		// 上面的意思是流对象，我们不关心它，低下的处理时非流对象的。
		
		//if (pStreamCtx->RefCount>0)//避免引用出现负数，但若这样写会出现蓝屏~ 
		//{
			if ((FltObjects->FileObject->Flags & FO_STREAM_FILE) != FO_STREAM_FILE)
			{
				pStreamCtx->RefCount -- ; // decrement reference count 若直接--就蓝屏，说明某个系统组件再占用着

				if ((pStreamCtx->bHasExcelWriteData == TRUE) && (bIsSystemProcess == EXCEL_PROCESS))//通过改变自减的时机的来解决Excel关闭时无法置零的问题
				{
					pStreamCtx->RefCount -- ;
					pStreamCtx->bHasExcelWriteData = FALSE;
				}
			...
			{
				if ((bIsSystemProcess == EXCEL_PROCESS) && (pStreamCtx->bHasExcelWriteData == FALSE))
				pStreamCtx->bHasExcelWriteData = TRUE;//直接蓝屏 if ObDereferenceObject(FltObjects->FileObject);
			}		
		...
		//}
		if(0 == pStreamCtx->RefCount)//Original 经过测试发现关闭文件后，什么都不做也会激活它，看到只有bEncryptOnWrite为1
		//if ((0 == pStreamCtx->RefCount)||((pStreamCtx->RefCount==1)&&((FltObjects->FileObject->Flags & FO_STREAM_FILE) != FO_STREAM_FILE)))//感觉这是必须的，否则没有写入文件标志的机会了~ For Excel~
		{//if reference decreases to 0, write file flag, flush|purge cache, and delete file context
			//当去掉exeploere.exe机密进程后，能为0了。
			...
			//bHasWriteData = 0 导致没有写上加密标志，所以研究write部分
			//加上 || pStreamCtx->bEncryptOnWrite 进行测试~ 
			if ((pStreamCtx->bHasWriteData || pStreamCtx->bIsFileCrypt) || (!pStreamCtx->bHasWriteData && !pStreamCtx->bIsFileCrypt && pStreamCtx->bHasPPTWriteData))
			{
				...
				DbgPrint("3 wszVolumePathName:%S\n", wszVolumePathName);
				...
				DbgPrint("6 dosFullPath & wszFileDosFullPath:%S\n", wszFileDosFullPath);
				...
				{//write flag into end of file
					...
					File_WriteFileFlag(Data,FltObjects, FileObject, pStreamCtx) ;//没有命中~~~
				}
				...
				//flush cache and purge data in cache 因为文件关闭时必须清理缓存并写回，所以开头不能使用引用为1的条件来写入。
				if(0 == pStreamCtx->RefCount)//if(bIsSystemProcess != EXCEL_PROCESS)	
				{//加上判断为了测试是否能解决EXCEL再次修改并保存时的崩溃问题~
					Cc_ClearFileCache(FileObject, TRUE, NULL, 0) ; 
					if(bIsSystemProcess == EXCEL_PROCESS)	
					DbgPrint("----- pStreamCtx->RefCount = 0 ----------------【 EXCEL_PROCESS Call Cc_ClearFileCache】---------------------\n");
				}
		...
		//这应该是引用为0后才可以删除流上下文。 bDeleteStreamCtx = TRUE ; //set flag to delete stream context before returning from pre-close routine
...
--*/
{
    ...
    try {
		...
		newBuf =ExAllocatePool(NonPagedPool,readLen);// 分配表头节点的空间
...
--*/
{
    ...
    try {
		...
		if (!pStreamCtx->bIsFileCrypt && !FltObjects->FileObject->WriteAccess)//文件没有加密 而且文件不可写
		{
		...
		newBuf =ExAllocatePool(NonPagedPool,writeLen);// 分配表头节点的空间
        if (newBuf == NULL) 
            leave;
			...
			do {
					 ...
					 IRP_NOCACHE|IRP_PAGING_IO|IRP_SYNCHRONOUS_PAGING_IO)))
				{
					...
					///{// use file specified key to encrypt file data(this must be history key)
						///if (data_decrypt(newBuf, writeLen))
						///{
						///	KeLeaveCriticalRegion() ;
						///	break ;
					///	}
    ...
    )
{
	...
	if (!p2pCtx->pStreamCtx->bHasWriteData)
	{
		...
		p2pCtx->pStreamCtx->bHasWriteData = TRUE ;//in PostWrite 有数据写入时会打上标志bHasWriteData
	}
	SC_UNLOCK(p2pCtx->pStreamCtx, OldIrql) ;

 	 if (!(Data->Iopb->IrpFlags & (IRP_NOCACHE | IRP_PAGING_IO | IRP_SYNCHRONOUS_PAGING_IO)))
         {
             //在NTFS下，如果应用未设置大小，直接写必须调用此才能将数据写回硬盘，否则，只有文件大小在变化，内容并未写到硬盘
			//if (bIsSystemProcess != EXCEL_PROCESS)
...
--*/
{
    ...
    try {
        ...
        newBuf =ExAllocatePool(NonPagedPool,iopb->Parameters.DirectoryControl.QueryDirectory.Length);// 分配表头节点的空间
		if (newBuf == NULL) 
            leave;
