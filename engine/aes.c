...
void aes_blockenc(struct aes_context ctx, unsigned char in[16], unsigned char out[16])
{
    memcpy( out, in, 16);
    aes_encrypt( &ctx, out);
}

void aes_blockdec(struct aes_context ctx, unsigned char in[16], unsigned char out[16])
{
    memcpy( out, in, 16);
    aes_decrypt( &ctx, out);
}
void aes_makekey(struct aes_context ctx, unsigned char *key)
{
